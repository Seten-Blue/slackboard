...
router.post('/sync-channels', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        for (const slackChannel of slackChannels) {
            let channel = await Channel_1.default.findOne({ slackChannelId: slackChannel.id });
            if (!channel) {
                channel = await Channel_1.default.findOne({ name: slackChannel.name });
            }
            if (!channel) {
                ...
                const changed = channel.name !== slackChannel.name || channel.displayName !== slackChannel.name;
                if (!channel.slackChannelId) {
        ...
        const currentIds = new Set(slackChannels.map((s) => s.id));
        const staleChannels = await Channel_1.default.find({ platform: 'slack', slackChannelId: { $ne: null }, slackTeamId: null });
        let prunedCount = 0;
        for (const ch of staleChannels) {
            if (ch.slackChannelId && !currentIds.has(String(ch.slackChannelId))) {
                ...
                await Channel_1.default.findByIdAndDelete(ch._id);
                prunedCount++;
            }
        }
        res.json({
        ...
        if (type === 'event_callback' && event) {
                    ...
                    if (channel) {
                        ...
                        if (nameChanged || displayChanged) {
                            if (nameChanged) {
                                ...
                                const taken = await Channel_1.default.findOne({ name: newName, _id: { $ne: channel._id } });
                                if (taken) {
            ...
            if (event.type === 'channel_deleted' && event.channel) {
                ...
                if (channel) {
                    ...
                    await Channel_1.default.findByIdAndDelete(channel._id);
                    const io = req.app.get('io');
                    if (io) {
